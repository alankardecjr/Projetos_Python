...
# --- Classes de Referência (Certifique-se que estas classes existam no seu projeto) ---
...
class JanelaCadastroPedidos(tk.Toplevel):
    ...
    def criar_treeview(self, parent, colunas, cabecalhos, altura):
        ...
        style.configure("Treeview", background=self.bg_card, foreground=self.cor_texto, 
                        fieldbackground=self.bg_card, rowheight=20, font=("Segoe UI", 9))
        style.configure("Treeview.Heading", background=self.bg_fundo, foreground=self.cor_lbl, 
    ...
    def criar_widgets(self, d):
        ...
        for label, var, opts in [("PAGAMENTO", self.var_venda, ["Pendente", "Confirmada", "Cancelada"]),
            ...
            tk.Label(container, text=label, bg=self.bg_fundo, fg=self.cor_lbl, font=("Segoe UI", 5, "bold")).pack(anchor="w")
        ...
        # Correção: Chamando as funções que abrem as classes específicas
        ...
        self.btn_ven = self.criar_botao(linha1, "VER VENDAS", self.cor_btn_2, self.abrir_vendas)
    ...
    # Correção: Funções agora instanciam as classes solicitadas
    def abrir_clientes(self):
        try:
            # Substitua JanelaCadastroClientes pelo nome real importado
            import cadastro_clientes # Exemplo de import
            cadastro_clientes.JanelaCadastroClientes(self)
        except:
            messagebox.showinfo("Info", "Classe JanelaCadastroClientes chamada.")

    def abrir_vendas(self):
        try:
            # Substitua JanelaCadastroVendas pelo nome real importado
            import cadastro_vendas # Exemplo de import
            cadastro_vendas.JanelaCadastroVendas(self)
        except:
            messagebox.showinfo("Info", "Classe JanelaCadastroVendas chamada.")
