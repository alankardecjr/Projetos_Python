...
import database  # Certifique-se que o database.py está na mesma pasta
...
class JanelaCadastroProdutos(tk.Toplevel):
    ...
    def criar_widgets(self):
        ...
        def ao_entrar_botao(e, cor_destaque): e.widget.config(bg=cor_destaque)
        def ao_sair_botao(e, cor_original): e.widget.config(bg=cor_original)
        ...
        # --- HEADER ---
        tk.Label(main_frame, text="Gerenciar Produto", bg=self.bg_fundo, 
                 fg=self.cor_texto, font=("Segoe UI", 14, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 15))
        ...
        self.btn_sair = tk.Button(main_frame, text="CANCELAR", bg=self.cor_btn_sair, fg="white", 
                                  font=("Segoe UI", 9, "bold"), width=35, relief="flat", cursor="hand2", 
                                  command=self.fechar)
        self.btn_sair.grid(row=14, column=0, columnspan=2, pady=(5, 0), ipady=8)
    ...
    def validar_e_salvar(self):
        ...
        if not all([d["produto"], d["cor"], d["venda"], d["tam"], d["qtd"]]):
            messagebox.showwarning("Atenção", "Preencha todos os campos marcados com (*).")
            return

        try:
            if self.produto_id:
                # Atualiza o item completo no banco
                database.atualizar_item(
                    self.produto_id, d["produto"], d["cor"], d["tam"], 
                    d["custo"], d["venda"], d["qtd"], d["cat"], d["forn"], d["status"]
                )
            ...
            messagebox.showerror("Erro", f"Erro ao processar banco de dados: {e}")

    def preencher_dados(self, d):
        """ Preenche os campos ao editar um produto existente """
        # Limpar campos antes de inserir para evitar duplicatas
        entries = [self.ent_produto, self.ent_cor, self.ent_tam, self.ent_custo, 
                   self.ent_venda, self.ent_qtd, self.ent_cat, self.ent_fornecedor]
        
        for i, entry in enumerate(entries, start=1):
            entry.delete(0, tk.END)
            entry.insert(0, d[i] if d[i] is not None else "")
            
        self.var_status.set(d[9])
        self.btn_salvar.config(text="ATUALIZAR DADOS", bg="#059669") # Verde sutil para modo edição

if __name__ == "__main__":
    # Teste rápido de interface
    root = tk.Tk()
    root.withdraw() 
    # Para testar novo cadastro: JanelaCadastroProdutos(root)
    # Para testar edição: JanelaCadastroProdutos(root, [1, "Sapatilha Luxo", "Preta", "35", "50.00", "120.00", "10", "Calçados", "Fabrica X", "Disponível"])
